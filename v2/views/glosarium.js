  ...
  root.innerHTML = `
    <div class="row-between mb-4">
      <div>
        ...
        <p class="t-muted t-sm">${unique.length} istilah</p>
      </div>
    </div>
    ...
    <div class="col gap-4">
      ${sortedLetters
        ...
        <div>
          ...
          <dl class="col" style="gap:var(--space-2);margin-top:var(--space-3)">
            ${byLetter[letter]
              .map(
                (e) => `
              <div>
                <dt class="t-sm" style="font-weight:600">${escapeHTML(e.term.replace(/_/g, " "))}</dt>
                <dd class="t-sm t-muted" style="margin-left:0">${escapeHTML(String(e.def).slice(0, 200))}</dd>
              </div>
            `
              )
              .join("")}
