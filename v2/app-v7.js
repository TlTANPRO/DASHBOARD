...
// ============================================================
// NOTION PROP WRAPPER
// Map app-level prop names + raw values ke Notion format
// ============================================================
const SELECT_FIELDS = new Set([
  "PIC", "PIC Penanggung Jawab", "Divisi", "Periode", "Satuan", "Status",
  "Status", "Tipe", "Kategori", "Prioritas", "Approval", "Quarter", "Frekuensi"
]);
const NUMBER_FIELDS = new Set([
  "Target", "Realisasi", "Progress (%)", "Progress", "Budget (Rp)", "budget",
  "Bobot (%)", "Tahun", "Actual Spend (Rp)"
]);
const DATE_FIELDS = new Set(["Tanggal", "Tanggal Mulai", "Mulai", "Deadline", "Approval_Time", "Effective From"]);
const TITLE_FIELDS = new Set(["KPI ID", "SOW ID", "Program ID", "Jobdesk ID"]);
const TITLE_LIKE_RICHTEXT = new Set(["KPI"]); // KPI is rich_text but semantically a name

function formatPropForNotion(name, val) {
  ...
  // Date (expect YYYY-MM-DD or ISO)
  if (DATE_FIELDS.has(name)) {
    const s = String(val);
    return { date: { start: s.length > 10 ? s : s } };
  }
  // Title
  if (TITLE_FIELDS.has(name)) {
    return { title: [{ text: { content: String(val) } }] };
  }
  // KPI name (rich_text but acts like a name)
  if (TITLE_LIKE_RICHTEXT.has(name)) {
    return { rich_text: [{ text: { content: String(val) } }] };
  }
  // Default: rich_text (Catatan, Jobdesk, Deskripsi, etc.)
  if (name === "Bukti" || name === "Approval_Time") {
    return { rich_text: [{ text: { content: String(val) } }] };
  }
  return { rich_text: [{ text: { content: String(val) } }] };
}

function formatPropsForNotion(props) {
  const out = {};
  for (const [k, v] of Object.entries(props)) {
    if (k === "id" || k === "createdAt" || k === "_editTime") continue;
    const wrapped = formatPropForNotion(k, v);
    if (wrapped) out[k] = wrapped;
  }
  return out;
}
...
const API = {
  ...
  async create(dbKey, properties) {
    const cfg = window.DASHBOARD_CONFIG;
    const wrapped = formatPropsForNotion(properties);
    if (cfg.mode === "live") {
      const dbId = cfg.databases[dbKey];
      const res = await fetch(cfg.workerBase + "/notion/v1/pages", {
        ...
        body: JSON.stringify({ parent: { database_id: dbId }, properties: wrapped })
      });
      if (!res.ok) {
        const txt = await res.text();
        throw new Error("Create gagal: " + res.status + " — " + txt.slice(0, 200));
      }
      return await res.json();
    } else {
  ...
  async update(dbKey, id, properties, editTime) {
    ...
    if (cfg.mode === "live") {
      ...
      const res = await fetch(cfg.workerBase + "/notion/v1/pages/" + id, {
        method: "PATCH",
        headers,
        body: JSON.stringify({ properties: wrapped })
      });
      if (res.status === 409) throw new Error("Data sudah diubah orang lain — refresh dulu");
      if (!res.ok) {
        const txt = await res.text();
        throw new Error("Update gagal: " + res.status + " — " + txt.slice(0, 200));
      }
      return await res.json();
    } else {
...
// Invalidate leaderboard cache whenever KPI data changes
function invalidateKpiCache() { leaderboardCache = { at: 0, rows: [] }; }
...
function kpiFormHTML(r = {}) {
  ...
  return `<form id="kpi-form">
    <div class="form-grid">
      <div class="form-row">
        <label>KPI ID
        <input class="input" name="KPI ID" value="${escapeAttr(r["KPI ID"] || r.kpiId || "KPI-2026-" + Date.now().toString().slice(-4))}" required />
        </label>
      </div>
      <div class="form-row">
        <label>KPI (nama)
        <input class="input" name="KPI" value="${escapeAttr(r.KPI || "")}" placeholder="Closing Bulanan / Lead Mingguan" />
        </label>
      </div>
      <div class="form-row">
        <label>PIC
      ...
      <div class="form-row">
        ...
        <select class="select" name="Tipe">
          ${["Personal", "Divisi"].map(s => `<option ${(r.Tipe || "Personal") === s ? "selected" : ""}>${s}</option>`).join("")}
        </select>
        </label>
      </div>
      <div class="form-row">
      ...
      <div class="form-row">
        ...
        <select class="select" name="Satuan">
          ${sel(r.Satuan || "%", ["%", "Unit", "Rp", "Closing", "Lead", "Jam", "BAST", "Konten", "Vendor", "Decision"])}
        </select>
        </label>
      </div>
      <div class="form-row">
    ...
    <div class="form-row">
      <label>Catatan
      <textarea class="textarea" name="Catatan">${escapeAttr(r.Catatan || "")}</textarea>
      </label>
    </div>
...
function progFormHTML(r = {}) {
  ...
  return `<form id="prog-form">
    <div class="form-grid">
      <div class="form-row">
        <label>Program ID
        <input class="input" name="Program ID" value="${escapeAttr(r["Program ID"] || r.programId || "PROG-2026-" + Date.now().toString().slice(-4))}" required />
        </label>
      </div>
      <div class="form-row">
        <label>PIC Penanggung Jawab
        <select class="select" name="PIC Penanggung Jawab" required>
          ${cfg.picList.map(p => `<option ${(r["PIC Penanggung Jawab"] || r.pic) === p ? "selected" : ""}>${p}</option>`).join("")}
        </select>
        </label>
      </div>
      <div class="form-row" style="grid-column:1/-1">
        <label>Nama Program
        <input class="input" name="Nama Program" value="${escapeAttr(r["Nama Program"] || r.nama || "")}" required />
        </label>
      </div>
      <div class="form-row">
        <label>Quarter
      ...
      <div class="form-row">
        <label>Tanggal Mulai
        <input class="input" type="date" name="Tanggal Mulai" value="${r["Tanggal Mulai"] || r.Mulai || todayISO()}" />
        </label>
      </div>
      <div class="form-row">
        <label>Deadline
      ...
      <div class="form-row">
        <label>Progress (%)
        <input class="input" type="number" min="0" max="100" name="Progress (%)" value="${r["Progress (%)"] || r.Progress || 0}" />
        </label>
      </div>
      <div class="form-row">
        <label>Status
      ...
      <div class="form-row">
        <label>Budget (Rp)
        <input class="input" type="number" name="Budget (Rp)" value="${r["Budget (Rp)"] || r.budget || 0}" />
        </label>
      </div>
    </div>
    <div class="form-row">
      <label>Risiko
      <textarea class="textarea" name="Risiko">${escapeAttr(r.Risiko || r.risiko || "")}</textarea>
      </label>
    </div>
    <div style="display:flex;gap:var(--space-2);justify-content:flex-end;margin-top:var(--space-4)">
      <button type="button" class="btn btn-ghost" id="form-cancel">Batal</button>
...
function jobFormHTML(r = {}) {
  ...
  return `<form id="job-form">
    <div class="form-grid">
      <div class="form-row">
        <label>Jobdesk ID
        <input class="input" name="Jobdesk ID" value="${escapeAttr(r["Jobdesk ID"] || r.jobdeskId || "JOB-" + todayISO().replace(/-/g, "") + "-" + (Session.pic || "X").split(" ")[0] + "-" + Date.now().toString().slice(-3))}" required />
        </label>
      </div>
      <div class="form-row">
        <label>PIC
      ...
      <div class="form-row">
        <label>Tanggal
        <input class="input" type="date" name="Tanggal" value="${r.Tanggal ? r.Tanggal.slice(0, 10) : todayISO()}" required />
        </label>
      </div>
      <div class="form-row">
        <label>Kategori
    ...
    <div class="form-row">
      <label>Jobdesk
      <textarea class="textarea" name="Jobdesk" required>${escapeAttr(r.Jobdesk || r.jobdesk || "")}</textarea>
      </label>
    </div>
    <div class="form-grid">
      <div class="form-row">
        <label>Target Output
        <textarea class="textarea" name="Target Output">${escapeAttr(r["Target Output"] || r.target || "")}</textarea>
        </label>
      </div>
      <div class="form-row">
        <label>Actual Output
        <textarea class="textarea" name="Actual Output">${escapeAttr(r["Actual Output"] || r.actual || "")}</textarea>
        </label>
      </div>
    </div>
    <div class="form-row">
      <label>Bukti (URL)
      <input class="input" type="url" name="Bukti" value="${escapeAttr(r.Bukti || "")}" />
      </label>
    </div>
    <div style="display:flex;gap:var(--space-2);justify-content:flex-end;margin-top:var(--space-4)">
      <button type="button" class="btn btn-ghost" id="form-cancel">Batal</button>
...
// ============================================================
// 02 LEADERBOARD (auto-compute dari KPI live Notion)
// ============================================================
...
async function renderLeaderboard() {
  ...
  if (!grid) return;
  if (!Session.pic) {
    grid.innerHTML = `<div class="empty-state"><h3>Login dulu</h3><p>Leaderboard butuh login untuk hitung score dari KPI.</p></div>`;
    return;
  }
  grid.setAttribute("aria-busy", "true");
  try {
    let allKPI = leaderboardCache.rows;
    if (Date.now() - leaderboardCache.at > LEADERBOARD_TTL_MS || allKPI.length === 0) {
      const res = await API.query("kpi");
      allKPI = res.results || [];
      leaderboardCache = { at: Date.now(), rows: allKPI };
    }
    ...
    allKPI.forEach(k => {
      if (k.Tipe === "Divisi") return; // skip Divisi-level, leaderboard only Personal
      const d = picToDivisi[k.PIC];
      const tgt = Number(k.Target) || 0;
      if (d && tgt > 0) byDivisi[d].push(k);
    });
    grid.style.cssText = "";
    ...
    for (const [key, list] of Object.entries(byDivisi)) {
      ...
      list.forEach(k => {
        const tgt = Number(k.Target) || 0;
        const real = Number(k.Realisasi) || 0;
        const skor = (real / tgt) * 100;
...
function refreshAll() {
  ...
  loadJobdesk();
  loadSOW();
  // renderLeaderboard: skip if hidden section
  if (document.querySelector("#leaderboard-section") && !document.querySelector("#leaderboard-section").hidden) {
    renderLeaderboard();
  }
